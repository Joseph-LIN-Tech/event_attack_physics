...
import argparse
from data_rvtclasses import dataclass, field
...
from utils_rvt.preprocessing import _blosc_opts
from data_rvt.utils.representations import MixedDensityEventStack, StackedHistogram, RepresentationBase
...
class H5Writer:
    def __init__(self, outfile: Path, key: str, ev_repr_shape: Tuple, numpy_dtype: np.dtype):
        ...
        self.h5f.create_dataset(key, dtype=self.numpy_dtype.name, shape=chunkshape, chunks=chunkshape,
                                maxshape=maxshape, **_blosc_opts(complib='blosc:lz4',complevel=1, shuffle='byte'))
        self.t_idx = 0
...
def get_base_delta_ts_for_labels_us(unique_label_ts_us: np.ndarray, dataset_type: str = 'gen1') -> int:
    print("enter get_base_delta_ts_for_labels_us")
    if dataset_type == 'gen1':
        delta_t_us_10hz = 100000 # us
        return delta_t_us_10hz # min requirement
    ...
    print("delta_t_us_approx_10hz",delta_t_us_approx_10hz)
...
def save_labels(out_labels_dir: Path,
    ...
    if outfile_labels.exists() and match_if_exists:
        ...
        print("labels_existing",labels_existing)
...
def labels_and_ev_repr_timestamps(npy_file: Path,
    ...
    ts_step_frame_ms = ts_step_ev_repr_ms
    ...
    sequence_labels = np.load(str(npy_file))#.astype("int") # bbox_npy file
    ...
    base_delta_ts_labels_us = align_t_us #get_base_delta_ts_for_labels_us(
        #unique_label_ts_us=unique_ts_us, dataset_type=dataset_type)
    print("base_delta_ts_labels_us",base_delta_ts_labels_us)
    ...
    print("unique_ts_idx_first",unique_ts_idx_first)
    ...
    for unique_ts_idx in range(unique_ts_idx_first + 1, len(unique_ts_us)):
        ...
        base_delta_count = round(diff_to_ref / base_delta_ts_labels_us)# delta_ts_us: 
    ...
    print("sequence_labels['t']",sequence_labels['t'])
    print("frame_timestamps_us",frame_timestamps_us)
    ...
    print("start_indices_per_label",start_indices_per_label)
    print("end_indices_per_label",end_indices_per_label)
    ...
    for idx_start, idx_end in zip(start_indices_per_label, end_indices_per_label):
        ...
        print("label_time_us",label_time_us)
...
def write_event_representations(in_h5_file: Path,
                                ...
                                ev_repr_timestamps_us: np.ndarray,
                                downsample_by_2: bool,
                                overwrite_if_exists: bool = True) -> None:
    ...
    #print("in_h5_file",in_h5_file)
    #print("dataset",dataset)
    #print("ev_outfile_in_progress",ev_outfile_in_progress)
...
def process_sequence(dataset: str,
    ...
    align_t_ms = ts_step_ev_repr_ms #lgx
    try:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-ts', '--ts_step_ev_repr_ms', type=int, default=50, help="Num proceesses to run in parallel")
    ...
    print("dataset_input_path ",dataset_input_path)
    ...
    ts_step_ev_repr_ms = ev_repr_delta_ts_ms #args.ts_step_ev_repr_ms #50  # Could be an argument of the script.
