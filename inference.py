...
class ESIM():
    ...
    def get_th(self, images, irradiance_maps):
        ...
        threshold_C_pos_origin = real_normal_sample * self.threshold['contrast_threshold_sigma_pos'] + self.threshold['contrast_threshold_pos'] # refer to equation 
        ...
        tmp = (threshold_C_pos_origin<minimum_threshold)# & random_samplef
    ...
    def get_constant_th(self,images,constrast_threshold_pos,constrast_threshold_neg):
        self.threshold_C_pos = torch.full(images.size(),constrast_threshold_pos).to(self.device)
        self.threshold_C_neg = torch.full(images.size(),constrast_threshold_neg).to(self.device) 

    def inferenceBefore(self, images):
        self.irradiance_maps_0 = self.ln_map(images)#image with the format in ln map values
        
        self.last_img = self.irradiance_maps_0.clone() # last ln map values
        self.ref_values = self.irradiance_maps_0.clone() # last ln map values
        self.current_time = 0
    ...
    def inference(self, images, now_time):
        ...
        # the difference betwwen irradiance_maps and last_img
        # ref_values is the multiple of threshold, but last_img is not 

        delta_time = now_time - self.current_time # self.current_time is the timestamp of last image
        irradiance_maps = self.ln_map(images)
        self.get_constant_th(images, self.pre_pos_threshold, self.pre_neg_threshold) 
        #self.get_th(images, irradiance_maps)
        
        irradiance_values_pos = F.relu(irradiance_maps-self.ref_values) // self.threshold_C_pos #to get the iteration number for pos event
        irradiance_values_neg = F.relu(-(irradiance_maps-self.ref_values)) // self.threshold_C_neg #get the iteration number for neg event
        
        irradiance_values_pos[self.ref_values+self.threshold_C_pos < self.last_img] = 0 # ignore the pixels that change less than threshold
        ...
        length = int(irradiance_values_pos.sum().item())+int(irradiance_values_neg.sum().item())+shotNoiseNumber # the length of quar-tuples
        ...
        t_pos = self.current_time+(self.ref_values-self.last_img)/(irradiance_maps-self.last_img + self.delta_min)*delta_time # 0  # delta_min is to avoid 0
        t_pos_iter = self.threshold_C_pos/(irradiance_maps-self.last_img + self.delta_min)*delta_time # the unit t periord of 1/threshold, ie., the time distribution is linear in delta_time
        t_pos_ori = t_pos.clone().detach()
        t_pos_iter_ori = t_pos_iter.clone().detach()
        
        t_neg = self.current_time+(self.ref_values-self.last_img)/(irradiance_maps-self.last_img + self.delta_min)*delta_time# 0
        t_neg_iter = self.threshold_C_neg/(irradiance_maps-self.last_img + self.delta_min)*delta_time # meaning the unit time of 1/threshold
        ...
        pos_event_xy = torch.where(irradiance_values_pos>self.delta_min)# Returns those indices whose brightness changes exceed a threshold
        ...
        self.ref_values += (irradiance_values_pos_ori * self.threshold_C_pos - irradiance_values_neg_ori * self.threshold_C_neg) # the current intensity images 
...
def main(args):
    ...
    output_h5_path = os.path.join(desPath, 'output.h5')
   #output_txt_path = output_h5_path.replace(".h5",".txt") 
    ep = hdf5_packager(output_h5_path)
    ...
    while(True):
        ...
        image = image[:,:,:args.size[0],:args.size[1]]
        ...
        event = esim.inference(image, time_stamp)# time_stamp is the timestamp of the current image
        #print("event",event)
   ...
   # readH5(output_h5_path)
    #vis_events_accumulation(output_h5_path.replace(".h5",".txt"), output_h5_path.replace(".h5",".png"), sensor_size[0], sensor_size[1])
