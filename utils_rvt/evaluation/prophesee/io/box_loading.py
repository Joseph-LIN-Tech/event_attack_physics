...
def to_prophesee(loaded_label_list: LOADED_LABELS, yolox_pred_list: YOLOX_PRED_PROCESSED) -> \
    ...
    for loaded_labels, yolox_preds in zip(loaded_label_list, yolox_pred_list):
        ...
        time = None
        # --- LOADED LABELS ---
        loaded_labels = loaded_labels.cpu().numpy()
        #'t', 'x', 'y', 'w', 'h', 'class_id',  'class_confidence'
        loaded_label_proph = np.zeros((len(loaded_labels),), dtype=BBOX_DTYPE)
        loaded_label_proph['t'] = loaded_labels[0][0]
        loaded_label_proph['x'] = loaded_labels[0][1]
        loaded_label_proph['y'] = loaded_labels[0][2]
        loaded_label_proph['w'] = loaded_labels[0][3]
        loaded_label_proph['h'] = loaded_labels[0][4]
        loaded_label_proph['class_id'] = loaded_labels[0][5]
        loaded_label_proph['class_confidence'] = loaded_labels[0][6]
        ...
        #     loaded_label_proph[name] = np.asarray(loaded_labels.get(name), dtype=BBOX_DTYPE[name])
        #     if name == 't':
        time = np.unique(loaded_labels[0][0])
        assert time.size == 1
        time = time.item()
        ...
        if num_pred > 0:
            ...
            yolox_pred_proph['x'] = np.asarray(yolox_preds[:, 0], dtype=BBOX_DTYPE['x']) # -> x_left
            yolox_pred_proph['y'] = np.asarray(yolox_preds[:, 1], dtype=BBOX_DTYPE['y']) ## -> y_left
            yolox_pred_proph['w'] = np.asarray(yolox_preds[:, 2] - yolox_preds[:, 0], dtype=BBOX_DTYPE['w']) # -> w
            yolox_pred_proph['h'] = np.asarray(yolox_preds[:, 3] - yolox_preds[:, 1], dtype=BBOX_DTYPE['h'])  # -> h
