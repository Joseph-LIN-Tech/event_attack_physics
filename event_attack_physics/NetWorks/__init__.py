...
def loadPretrainedParameter(args, model):
    new_params = model.state_dict()
    '''
    ...
    if args.debug:
        ...
        #    print(k)
    for k in pretrain_dict:
        showMessage(fileName='__init__.py', functionName='loadPretrainedParameter', lineNumber=18, variableName='pretrain_dict',
                    variableValue=k)
    new_params.update(pretrain_dict)
    model.load_state_dict(new_params)
    #model = torch.load('xx.pth')
...
def create_model(args,config):
    from .attack_basic_model import attack_basic_model as M
    m = M(args,config)
